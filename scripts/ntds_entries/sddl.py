...
def parse_ntSecurityDescriptor(input_buffer):
	...
	for k, v in zip(fields, struct.unpack('<HHIIII', input_buffer[:20])):
		out[k] = v
...
def parse_acl(input_buffer):
	...
	for k, v in zip(fields, struct.unpack('<HHI', input_buffer[:8])):
		out[k] = v
	...
	while len(out) < count:
		...
		try:
			...
			if ace['Type'].endswith('Object'):
				...
				for k, v in zip(fields, struct.unpack('<I', input_buffer[8:12])):
					ace[k] = v
...
def parse_sddl_dacl_ace_type(ace_type):
	...
	"""
	return ACEType.get(ace_type, "Unknown ACE Type")